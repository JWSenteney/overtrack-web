...
export class TimelineComponent implements OnChanges, AfterViewInit {
	...
	@ViewChild("timelineBody", { static: true })
	element: ElementRef;
	@ViewChild("twitchEmbed", { static: true })
	twitchEmebed: ElementRef;
	...
	createTwitchPlayer() {
		...
		this.twitchVideoStart =
			Number(this.twitchURL.split("=")[1].split("s")[0]) +
			this.twitchVideoOffset;
		...
		this.twitchEmebed.nativeElement.children[0].addEventListener(
			"mouseover",
			e => {
			...
			if (now < this.twitchVideoStart + this.stage.end / 1000) {
				...
				if (this.twitchPlayer.isPaused()) {
					this.onPlay.push(() => {
						...
						this.twitchPlayer.seek(
							Math.max(
								this.twitchVideoStart + this.stage.start / 1000,
								now - 0.25
							)
						);
						this.ready = true;
	...
	createTimeline() {
		...
		const players = this.host
			.select(".timeline-players")
			.selectAll("div")
			.data(this.stage.players)
			.enter();
		...
		div
			...
			.attr("class", (player: Player) => {
				return (
					"col-xs-2 text-right text-" +
					player.colour +
					(player.name.indexOf("@") != -1 ? " too-long" : "")
				);
			})
			.text((player: Player) => player.name);

		const svg = div.append("svg").attr("class", "timeline col-xs-9");
		...
		// ults
		svg
			.selectAll(".timeline-event-nohover .ult-period")
			.data((player: Player) =>
				player.events.filter(event => event.type == "ult")
			)
			.enter()
			.append("rect")
			.attr(
				"class",
				(event: GameEvent) => "timeline-event ult-period " + event.id
			)
			.attr("y", "28px")
			.attr("height", "11px")
			.attr("fill", "#9FECFC")
			...
			.attr(
				"width",
				(event: GameEvent) =>
					this.x(Math.min(event.duration, this.stage.duration - event.time)) +
					"%"
			)
		...
		svg
			.selectAll(".timeline-event-nohover .ult-icon-gain")
			.data((player: Player) =>
				player.events.filter(event => event.type == "ult")
			)
			.enter()
			...
			.attr(
				"class",
				(event: GameEvent) => "timeline-event ult-icon-gain " + event.id
			)
			...
			.attr("height", "29px");
		svg
			.selectAll(".timeline-event-nohover .ult-icon-use")
			.data((player: Player) =>
				player.events.filter(
					event =>
						event.type == "ult" &&
						event.absoluteTime + event.duration < this.stage.end
				)
			)
			.enter()
			.append("image")
			.attr(
				"class",
				(event: GameEvent) => "timeline-event ult-icon-use " + event.id
			)
			.attr("xlink:href", "assets/images/timeline/ult_use.png")
			.attr(
				"x",
				(event: GameEvent) => this.x(event.time + event.duration) + "%"
			)
			.attr("transform", "translate(-14)")
			.attr("y", "18px")
			.attr("height", "29px");
		...
		svg
			.selectAll(".timeline-event .destroyed")
			.data((player: Player) =>
				player.events.filter(event => event.type == "destroyed")
			)
			.enter()
			.append("image")
			.attr(
				"class",
				(event: GameEvent) => "timeline-event destroyed " + event.id
			)
		...
		svg
			.selectAll(".timeline-event .destruction")
			.data((player: Player) =>
				player.events.filter(event => event.type == "destruction")
			)
			.enter()
			.append("text")
			.attr(
				"class",
				(event: GameEvent) => "timeline-event destruction " + event.id
			)
			.attr("x", (event: GameEvent) => this.x(event.time) + "%")
			.attr("y", 26)
			.attr("data-other", (event: GameEvent) => event.other)
		...
		svg
			.selectAll(".hero-image")
			.data((player: Player) => player.heroesPlayed)
			.enter()
			.append("image")
			.attr(
				"xlink:href",
		...
		svg
			.selectAll(".timeline-event .assist, .timeline-event .support-assist")
			.data((player: Player) =>
				player.events.filter(
					event => event.type == "assist" || event.type == "support-assist"
				)
			)
			.enter()
			.append("text")
			.attr(
				"class",
				(event: GameEvent) => "timeline-event " + event.type + " " + event.id
			)
			.attr("x", (event: GameEvent) => this.x(event.time) + "%")
			.attr("y", 20)
			.attr("data-other", (event: GameEvent) => event.other)
		...
		svg
			.selectAll(".timeline-event .ability-assist")
			.data((player: Player) =>
				player.events.filter(event => event.type == "ability-assist")
			)
			.enter()
			.append("text")
			.attr(
		...
		svg
			.selectAll(".timeline-event .kill")
			.data((player: Player) =>
				player.events.filter(event => event.type == "kill")
			)
			.enter()
			.append("text")
		...
		svg
			.selectAll(".timeline-event .death")
			.data((player: Player) =>
				player.events.filter(event => event.type == "death")
			)
			.enter()
			.append("image")
		...
		svg
			.selectAll(".timeline-event .death")
			.data((player: Player) =>
				player.events.filter(event => event.type == "resurrect")
			)
			.enter()
			.append("image")
			.attr(
				"class",
				(event: GameEvent) => "timeline-event resurrect " + event.id
			)
		...
		const b = div.append("b").attr("class", "col-xs-1 text-left num");
		b.append("span")
			.attr("class", "kill")
			.text((player: Player) => player.kills);
		b.append("span").text((player: Player) => " / ");
		b.append("span")
			.attr("class", "death")
			.text((player: Player) => player.deaths);

		const time = this.host
			.select(".timeline-players")
			.insert("div", ":first-child")
			.attr("class", "col-xs-12");
		time.append("div").attr("class", "col-xs-2");
		const timelineTickSvg = time
			.append("svg")
			.attr("height", 20)
		...
		const timelineAxis = timelineTickSvg
			.append("g")
		...
		const timelineAxis2 = timelineTickSvg
			.append("g")
			.attr("transform", "translate(0,19)")
			.call(axis);
		...
		window.setTimeout(e => {
			...
			svg.selectAll("svg .timeline-event").on("click", e => {
				if (this.active) {
			...
			svg.selectAll("svg rect").on("click", e => {
				let d3 = D3;
				if (this.active) {
					let x = D3.event.layerX / (lineArea2.node() as any).clientWidth;
					this.seek(x * this.stage.duration);
			...
			svg.selectAll("svg .hero-image").on("click", e => {
				let d3 = D3;
				if (this.active) {
					let x = D3.event.layerX / (lineArea2.node() as any).clientWidth;
					this.seek(x * this.stage.duration);
	...
	mouseOverTimelineEvent() {
		...
		let classes: Array<String> = D3.select(this as any)
			.attr("class")
			.split(" ");
		let eventId = null;
		...
		if (isKillFeed) {
			D3.selectAll("svg .timeline-event:not(." + eventId + ")").classed(
				"faded",
				true
			);
			D3.selectAll("tr.timeline-event:not(." + eventId + ")").classed(
				"slight-faded",
				true
			);
		} else {
			D3.selectAll(".timeline-event:not(." + eventId + ")").classed(
				"faded",
				true
			);
		}
	...
	widthPercentage(hero: GameHero) {
		return (
			(100 * (hero.end - hero.start)) / (this.stage.end - this.stage.start)
		);
	}
	...
	rightColor(kill: KillFeedEntry) {
		let isBlue =
			(kill.isLeftRed && !kill.isRes) || (!kill.isLeftRed && kill.isRes);
		if (isBlue) {
			return "text-blue";
		}
		return "text-red";
	...
	pushingEvents(events: any) {
		...
		return events.filter(
			e => e.type == "pushing" && e.end - e.start > 3 * 1000
		);
	}
	...
	tickEvents(events: any) {
		...
		for (let e of events) {
			...
			if (
				e.time - this.stage.start < 25 * 1000 ||
				this.stage.end - e.time < 25 * 1000
			) {
	...
	pushingWidthPercentage(event) {
		return (
			(100 * (event.end - event.start)) / (this.stage.end - this.stage.start)
		);
	}
	...
	highlightKill(kill: KillFeedEntry, time: number) {
		...
		return (
			-1 * 1000 < timeSinceKill && timeSinceKill < this.highlightKillWindow
		);
	}
}
