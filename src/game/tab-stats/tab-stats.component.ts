...
import {
	...
	heroGraphedStats,
	statNames
} from "../tab-graphs/tab-graphs.component";
...
export class HeroStatisticsComponent {
	...
	showPercent(index: number): boolean {
		let statName = heroStatNames[this.stat.heroName][index];
		return (
			statName.indexOf("accuracy") != -1 ||
			statName.indexOf("average") != -1 ||
			statName.indexOf("uptime") != -1
		);
	}
...
export class TabStatisticsComponent {
	...
	ngOnInit(): void {
		...
		if (this.endgameStatistics && this.heroPlayed) {
			...
			console.log(
				...
				this.heroPlayed.timePlayed
			);
			for (let hero of this.heroPlayed.timePlayed) {
				console.log(">>>", hero);
				if (hero.duration < 0.9 * 60 * 1000) {
					console.log(
						"Ignoring %d seconds on %s",
						hero.duration / 1000,
						hero.hero
					);
					continue;
				}
				let heroStats = this.makeHeroStats(
					hero.hero.split("_")[0],
					hero.duration,
					this.endgameStatistics.heroes,
					this.heroStatistics
				);
				if (heroStats) {
			...
			let summedDuration = this.heroPlayed.timePlayed.reduce(
				(p, c) => p + c.duration,
				0
			);

			// try to find all heroes in endgame stats
			let allheroStats = this.makeHeroStats(
				"all heroes",
				summedDuration,
				this.endgameStatistics.heroes,
				null
			...
			for (let stat of this.heroStatistics) {
				if (stat.heroName == "ALL" && this.heroPlayed) {
					...
					console.log(
						"Updating time played for ALL from",
						stat.timePlayed,
						"(from tab) to",
						this.heroPlayed.duration,
						"(from hero played)"
					);
		...
		this.heroNames = this.heroNames.sort((a, b) => {
			return (
				this.statsByHero.get(b).timePlayed - this.statsByHero.get(a).timePlayed
			);
		});

		// take the duration from "All Heroes"
		if (this.heroNames.indexOf("ALL") != -1) {
			this.duration = Array.from(this.statsByHero.values())
				.map(s => s.timePlayed)
				.reduce((p, c) => Math.max(p, c), 0);
		}

		let neverSwitched = Array.from(this.statsByHero.values()).reduce(
			(p, c) =>
				p || (c.heroName != "ALL" && c.timePlayed >= this.duration * 0.95),
			false
		);
	...
	makeHeroStats(
		hero: string,
		timePlayed: number,
		endgameStats: Array<{ hero: string; statistics: Map<string, number> }>,
		tabStats: Array<HeroStatistics>
	): HeroStatistics {
		console.log("Getting stats for", hero, "using endgame and tab stats");
		let matchingEndgameStats: {
			hero: string;
		...
		if (matchingEndgameStats) {
			console.log(
				"Found endgame stats:",
				matchingEndgameStats.statistics.keys()
			);
			...
			if (hero.toLowerCase().indexOf("all") == -1) {
				...
				for (let stat of Array.from(matchingEndgameStats.statistics.keys())) {
					...
					if (i != -1) {
						...
						console.error(
							"Could not find mapping for stat",
							hero,
							'"' + statName + '"'
						);
					}
			...
			for (let k of Object.keys(statNameLookup)) {
				console.log(
					...
					statNameLookup[k],
					"->",
					matchingEndgameStats.statistics.get(k)
				);
