	...
	addPlayersToStage(
		...
		for (const player of team) {
			...
			for (const event of killfeedAndTab) {
				if (
					event.time > stage.end ||
					(event.time < stage.start && stage.index != 0)
				) {
				...
				if ("hero" in event) {
					...
					if (
						tabPress.blue == (teamColour == "blue") &&
						team[tabPress.player] == player
					) {
						hero = tabPress.hero;
					}
				} else {
					...
					if (
						kill.rightPlayer === player.name &&
						((!kill.isRes && leftColor != teamColour) ||
							(kill.isRes && leftColor == teamColour))
					) {
						if (kill.isRes) {
							hero = kill.rightHero;
			...
			if (kill.assists) {
				for (let assist of kill.assists) {
					...
					if (assister) {
						...
						console.warn(
							"Could not find player assisting as",
							assist,
							"for kill at",
							kill.time
						);
	...
	toGame(res: Response): Game {
		...
		for (const stage of objective_stages) {
			...
			this.addPlayersToStage(
				...
				"blue",
				body.tab_statistics,
				body.spectate_bar ? null : heroPlayed
			);
			this.addPlayersToStage(
				players,
				stage,
			...
			if (
				...
				stage.end - stage.checkpoints[stage.checkpoints.length - 1][0] <
					10 * 1000
			) {
				// && progress == 0){
		...
		if (body.hero_statistics) {
			...
			if (heroStatistics.length == 1 && heroStatistics[0].heroName == "ALL") {
				let mainPlayed = heroPlayed.timePlayed.sort(
				...
				if (mainPlayed.percent > 0.95) {
					console.warn(
						mainPlayed.hero,
						"was played for",
						mainPlayed.percent * 100,
						'% of the game but only "ALL" was seen - setting ALL at',
						mainPlayed.hero
					);
