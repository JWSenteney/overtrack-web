...
export class EditGameComponent {
	...
	constructor(private http: Http, @Inject(DOCUMENT) public document: any) {}
	...
	edit() {
		...
		if (playername && playername != this.game.player) {
			if (
				!confirm(
					"Changing the player name will change the tab this game belongs to in the games list. Are you sure?"
				)
			) {
			...
			if (this.game.stages) {
				...
				// hack hack hack
				$(".timeline-players > .col-xs-12 > .text-blue").get(
					0
