...
export class AccountsDropdownComponent implements OnInit {
		...
		if (allSelected && specificAccountSelected) {
			if (this.allPreviouslySelected) {
				...
				this.selected = this.selected.filter(
					s => s == "All Accounts" || s == "Custom Games"
				);
			}
		}
	}
	...
	dropdownClosed() {
		if (
			this.selected.sort().toString() != this.lastNotified.sort().toString()
		) {
...
export class IntegrationsComponent implements OnInit {
	...
	ngOnInit(): void {
		this.route.queryParams.subscribe((params: Params) => {
			...
			if (postArgs) {
				...
				response = this.http.post(this.integrationsURL, postArgs, options);
			} else if (this.loading) {
				let options = new RequestOptions({ withCredentials: true });
				response = this.http.get(this.integrationsURL, options);
			}
	...
	update(res) {
		...
		this.webhooks = this.integrations.reduce((prev, cur) => {
			return (
				prev +
				Number(cur.type == "DISCORD_WEBHOOK" || cur.type == "GENERIC_WEBHOOK")
			);
		}, 0);
		$("#integrations")
			.find("button")
	...
	linkTwitch() {
		let url =
			...
			"%3Fintegrations%3Dtwitch" +
			"&response_type=code" +
			"&scope=" +
			this.twitchOAuthScope +
			"&nonce=" +
			this.twitchOAuthNonce;
