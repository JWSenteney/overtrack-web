...
export class GamesListComponent implements OnInit, AfterContentChecked {
	...
	constructor(
		...
		@Inject(DOCUMENT) public document: any // private cdr: ChangeDetectorRef
	) {}
	...
	ngAfterContentChecked(): void {
		...
		if (this.isOwnGames) {
			content =
				'Please <a href="/subscribe">subscribe</a> to view full match details for games played after the trial period has ended';
		} else {
			content =
				"In order to view full match details the owner of this game needs to be subscribed";
		}
		...
		this.gamesListService.fetchSharedGames(
			...
			(games, seasons) => {
				...
				if (this.gamesLists.length) {
					this.accountNames = this.gamesLists
						.map(gl => gl.player.toUpperCase().split("#")[0])
						.filter(e => e != "CUSTOM GAMES");
	...
	updateUploadingGame(user: User) {
		if (
			user &&
			user.currentUploadRequested &&
			new Date().getTime() - user.currentUploadRequested.getTime() <
				10 * 60 * 1000
		) {
	...
	updateGamesList() {
				...
				for (let g of gl.list) {
					switch (g.role) {
						case "tank":
							...
							break;
						case "damage":
							if (!this.playedDamage) {
								this.numRolesPlayed++;
							}
							this.playedDamage = true;
							break;
						case "support":
							if (!this.playedSupport) {
								this.numRolesPlayed++;
							}
		...
		if (this.numRolesPlayed == 1) {
			if (this.playedTank) {
				this.roleFilter = "tank";
			}
			if (this.playedDamage) {
				this.roleFilter = "damage";
			}
			if (this.playedSupport) {
				this.roleFilter = "support";
			}
		} else {
		...
		for (let gl of this.gamesLists) {
			if (gl.player == this.player) {
				for (let g of gl.list) {
					if (
						!this.roleQueueEnabled ||
						this.roleFilter === "all" ||
						this.roleFilter === g.role
					) {
						visibleGames.push(g);
					}
				}
		...
		for (let game of games) {
			...
			if (game.endSR == null) {
				...
				if (
					currentSR[gameRole] !== null &&
					currentSR[gameRole] !== game.startSR
				) {
		...
		plot.on("plotly_click", function(data) {
			if (index2id.get(data.points[0].pointNumber)) {
				const element = document.getElementById(
					"game-" + index2id.get(data.points[0].pointNumber)
				);
				const player = D3.select("#gametable li.active a").text();
				if (player !== this.player) {
					D3.select("#gametable li.active").classed("active", false);
					for (const elem of (D3.selectAll("#gametable li") as any)
						._groups[0]) {
						const d3elem = D3.select(elem);
						if (
							d3elem.select("a").size() &&
							d3elem.select("a").text() === this.player
						) {
	...
	batchEdit(action: string) {
		...
		let playername = $("#playername-input")
			.val()
			.toUpperCase();
		...
		if (action == "delete") {
			...
			$("#playername-input").fadeTo(100, 0.3, function() {
				$(this).fadeTo(500, 1.0);
			});
			return;
		}
