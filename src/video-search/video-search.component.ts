...
export class VideoSearchComponent implements OnInit {
	...
	private metadataEndpoint =
		"https://api2.overtrack.gg/video_search/metadata.vtt";
	...
	@ViewChild(VgHLS, { static: true })
	private vgHls: VgHLS;
	@ViewChildren(MultiSelectComponent)
	private dropdowns: QueryList<MultiSelectComponent>;
	// @ViewChildren(TextTrack) tracks : QueryList<TextTrack>;
	tracks: Array<TextTrack>;

	@ViewChild("heroDropdown", { static: true })
	private heroDropdown: MultiSelectComponent;
	...
	private typeDropdown: MultiSelectComponent;
	...
	@ViewChild("otherHeroDropdown", { static: true })
	private otherHeroDropdown: MultiSelectComponent;
	...
	ngOnInit(): void {
		this.http
			.get(this.heroesEndpoint, { withCredentials: true })
			.subscribe(r => {
	...
	onPlayerReady(api: VgAPI) {
		...
		(<any>this.vgHls).onError.subscribe(e => {
			...
			if (
				e.event == "hlsError" &&
				e.data &&
				e.data.type == "networkError" &&
				e.data.networkDetails.status == 404
			) {
	...
	removeType(id: string) {
		...
		if (this.form.get("type").value) {
			this.form
				.get("type")
				.setValue(this.form.get("type").value.filter(e => e.id != id));
		}
	}
	...
	updateDropdowns() {
		...
		for (let hero of this.eventsSummary) {
			...
			if (
				!this.form.get("hero").value ||
				this.form.get("hero").value.length == 0 ||
				this.dropdownContains("hero", hero.id)
			) {
	...
	onDropDownClose() {
		let hasOpenDropdown = this.dropdowns.reduce(
			(prev, dropdown) => prev || dropdown._settings.defaultOpen,
			false
		);
		if (hasOpenDropdown) {
			return;
