...
export class GamesGraphComponent implements AfterViewInit {
	...
	graphableGamesLists(gameLists: Array<PlayerGameList>) {
		...
		return gameLists.filter(
			gameList => gameList.list.filter(game => game.endSR != null).length >= 2
		);
	}
	...
	renderGraph(gameLists: PlayerGameList[]): void {
		...
		const unfilteredGames = gameLists.map(x => ({
			...
			games: x.list
				...
				.map(data => ({
					...
					date: data.startTime,
					sr: data.endSR || null,
					srWasUnknownReason: (data.endSR
						? null
						: data.rank == "placement"
						? "placement"
						: "unknown") as SRUnknownReason
				}))
		...
		const gamesWithXs = graphableGames.map(x =>
			x.games.map(game => ({
				x: NaN,
				...game
			}))
			...
			const outlineRun = () => {
				...
				for (const game of runOfUnknownSRGames) {
					plotlyLayoutShapes.push({
						...
						layer: "below",
						fillcolor:
							game.srWasUnknownReason == "placement"
								? "rgba(0, 127, 255, 0.25)"
								: "rgba(255, 0, 0, 0.125)"
					});
				}
		...
		const getRanges = (
			left: number,
			right: number,
			enabledTraces: Array<string> | null
		...
		const intitialLeft =
			allXs[Math.max(allXs.length - initialGamesVisible, 0)] - 0.5;
		...
		plotlyElement.on("plotly_click", eventData => {
			...
			const game: FullyAnnotatedGame =
				eventData.points[0].data.overtrackGames[
					eventData.points[0].pointNumber
				];
		...
		plotlyElement.on("plotly_relayout", eventData => {
			...
			if (eventSource == "user" && right != undefined && left != undefined) {
				const enabledTraces: Array<string> = plotlyElement.data
					.filter(e => e.showlegend == false && e.visible != "legendonly")
					.map(e => e.name);
	...
	fillUnknownSRs<T extends { data: Game; sr: number }>(games: T[]): T[] {
		...
		const srPerMatch = 25;
		const delta = (
			r: typeof Game.prototype.result,
			winCoefficient: number = 1.0,
			lossCoefficient: number = 1.0
		) => {
			if (r == "WIN") {
		...
		for (const entry of filled) {
			if (currentSegment) {
				if (entry.sr) {
					...
					currentSegment.end =
						entry.data.startSR || entry.sr - (delta(entry.data.result) || 0);
					currentSegment = null;
				} else {
		...
		for (const unknown of unknownSegments) {
			...
			if (wins > 0 && losses > 0) {
				...
				for (const games of unknown.games) {
					sr +=
						delta(games[0].data.result, winCoefficient, lossCoefficient) || 0;
					for (const game of games) {
						game.sr = Math.round(sr);
					}
				}
