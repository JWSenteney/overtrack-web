...
export class SubscribeComponent implements OnInit {
	...
	private cancelSubscriptionURL =
		"https://api.overtrack.gg/cancel_subscription";

	paypalSubscribeURL = "https://www.paypal.com/cgi-bin/webscr";
	paypalHostedButtonID = "M4B2GQMPY5N78";
	paypalUnsubscribeURL =
		"https://www.paypal.com/cgi-bin/webscr?cmd=_subscr-find&alias=M6LFV3XCTA2X6";
	...
	updateSubscriptionInfo() {
		this.http.get(this.subscriptionURL, { withCredentials: true }).subscribe(
			res => {
				let body = res.json();
				...
				if (body.trial_end_time) {
					...
					this.remainingDays = Math.round(
						(trialEndTime.getTime() - new Date().getTime()) /
							(1000 * 60 * 60 * 24)
					);
				}
				this.subscription = {
	...
	doSubscribe() {
		...
		var handler = (<any>window).StripeCheckout.configure({
			...
			token: token => {
				...
				this.http
					.post(this.subscribeURL, JSON.stringify(token), options)
					.subscribe(
						res => {
							this.updateSubscriptionInfo();
						},
	...
	cancelSubscription() {
		...
		this.http.post(this.cancelSubscriptionURL, "", options).subscribe(
			res => {
				this.updateSubscriptionInfo();
			},
			err => {
