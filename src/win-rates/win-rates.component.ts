...
import {
	...
	PlayerGameList,
	GamesListHero
} from "../games-list/games-list.service";
...
export class WinRatesComponent implements OnInit {
	...
	updateHeroWR(
		...
		thisHeroWR.significantGamesPlayed +=
			hero.percentagePlayed * 100 > this.lfhp ? 1 : 0;
		if (game.result === "WIN") {
			thisHeroWR.timewon += hero.percentagePlayed * game.duration;
			thisHeroWR.gamesWon += 1;
			thisHeroWR.significantGamesWon +=
				hero.percentagePlayed * 100 > this.lfhp ? 1 : 0;
		}
	}
	...
	calcWinrates() {
		...
		for (let game of this.currentPlayer.list) {
			...
			for (let singleHero of game.heroes) {
				...
				//all maps
				this.updateHeroWR(
					maps.get(ALL_MAPS_NAME).heroWinrates,
					game,
					singleHero
				);
			}
		...
		let mapcount = 0;
		while (
			Array.from(maps.keys()).length >
			this.mapList.length + this.mapList2.length
		) {
			let minMap = "";
			for (let keyMap of Array.from(maps.keys())) {
				...
				if (minMap === "" || keyMap < minMap) minMap = keyMap;
			}
			if (mapcount <= 8)
				//first 8 maps to first list
				this.mapList.push(minMap);
			else this.mapList2.push(minMap);
			mapcount++;
		}
	...
	currentMapHeroes() {
		...
		if (arr.indexOf("unknown") != -1) arr.splice(arr.indexOf("unknown"), 1);
		return arr;
	}
	...
	sortedByPlaytime(heroes: Array<string>) {
		...
		return heroes.sort((a, b) => {
			return (
				mapStat.heroWinrates.get(b).timeplayed -
				mapStat.heroWinrates.get(a).timeplayed
			);
		});
	}
	...
	getHeroWinrateByTime(heroName: string) {
		...
		if (heroWinrates.get(heroName).timewon == 0) return 0;
		return (
			(heroWinrates.get(heroName).timewon /
				heroWinrates.get(heroName).timeplayed) *
			100
		);
	}
	...
	getHeroWinrateByGames(heroName: string) {
		...
		// 	return heroWinrates.get(heroName).gamesWon / heroWinrates.get(heroName).gamesPlayed * 100;
		// }
		if (heroWinrates.get(heroName).significantGamesWon == 0) return 0;
		return (
			(heroWinrates.get(heroName).significantGamesWon /
				heroWinrates.get(heroName).significantGamesPlayed) *
			100
		);
	}
	...
	getHeroPlayrate(heroName: string) {
		let mapStat = this.mapStats.get(this.activeMap);
		return (
			(mapStat.heroWinrates.get(heroName).timeplayed / mapStat.maxHeroTime) *
			100
		);
	}
	...
	hideHero(heroName: string) {
		...
		if (
			!this.normalise &&
			map.heroWinrates.get(heroName).significantGamesPlayed == 0
		) {
			// no games played
			return true;
			...
			return (
				map.timeplayed / this.lfhp > map.heroWinrates.get(heroName).timeplayed
			);
		}
	}
